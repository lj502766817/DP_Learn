...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:  # 这里是为了处理windows和linux的逻辑,把路径下的所有东西都装到f里
            ...
            self.im_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in IMG_FORMATS)  # 把图片滤出来
        ...
        # Check cache
        self.label_files = img2label_paths(self.im_files)  # labels 根据根据图像地址拿到label的地址
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_suffix('.cache')
        try:  # 创建缓存,方便下次训练,先去加载,加载不到就创建
    ...
    def __getitem__(self, index):  # 拿数据的方法
        ...
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=img.shape[1], h=img.shape[0], clip=True, eps=1E-3)  # 把label从xyxy转成xywh的相对值(与真实图w,h的比值)
        ...
        if nl:
            labels_out[:, 1:] = torch.from_numpy(labels)  # label最前面加了个0
    ...
    def load_mosaic(self, index):
        ...
        return img4, labels4  # 这里label是,cls,xyxy
